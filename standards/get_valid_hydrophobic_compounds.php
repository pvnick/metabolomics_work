...
$compoundListContents = file_get_contents("super_hydrophobic");
...
foreach($compounds as $compoundEntry) {
    ...
    if (count($items) == 2) {
        ...
        if ($isValid) {
            $name=system("python -c \"import imp;nameresolver = imp.load_source('nameresolver', '../build_model/get_keggid_name.py');print(nameresolver.getNames('" . $keggid . "'))\"");
            echo($name . "\n");
        }
    }
}
